...
function showCard() {
    for (let i = 0; i < products.length; i++) {
        document.getElementById("products").innerHTML += `<div class="col-md-4"><div class="card ms-5 mb-5" style="width: 20rem;">
                <img src="${products[i].image}" class="card-img-top" alt="...">
                <div class="card-body">
                    <h5 class="card-title">${products[i].name}</h5>
                    <p class="card-text">${products[i].category}</p>
                    <a data-bs-toggle="modal" data-bs-target="#infoModal" class="btn btn-dark w-100" onclick="setModal(${i})">See more</a>
                </div>
            </div>
            </div>`;
    }
...
function setModal(index) {
    ...
    document.getElementById("mBody").innerHTML = `<p><b>Serial Number</b>: ${products[index].id}</p><p><b>Category</b>: ${products[index].category}</p><p><b>Description</b>: ${products[index].description}</p>
